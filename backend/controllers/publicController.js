...
export const submitContactInquiry = async (req, res) => {
  try {
    ...
    if (!normalizedPhone) {
      return res.status(400).json({ message: "Invalid Indian phone number. Must be a 10-digit mobile number starting with 6, 7, 8, or 9." });
    }
    ...
    // Send notification (simulating message delivery to admin email)
    if (SENDER_EMAIL) {
      await sendNotificationEmail(SENDER_EMAIL, `Contact Inquiry from ${name.trim()} (${normalizedPhone}): ${trimmedMessage}`);
    }
