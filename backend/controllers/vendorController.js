...
export const requestOtp = async (req, res) => {
  ...
  try {
    const vendor = await User.findOne({ email: normalizedEmail, role: "vendor" });
    if (!vendor) return res.status(404).json({ message: "Vendor not found" });

    const otpRecord = await OTP.findOne({ email: normalizedEmail, role: "vendor" });
    const now = new Date();
...
export const verifyOtp = async (req, res) => {
  ...
  try {
    const otpRecord = await OTP.findOne({ email: normalizedEmail, otpCode: otp, role: "vendor" });
  ...
  try {
    ...
    try {
      ...
      if (user && user.email) {
        ...
        const message = `Your booking for <strong>${itemName}</strong> has been updated to <strong>${status}</strong>.`;
