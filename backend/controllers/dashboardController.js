...
export const approveService = async (req, res) => {
  ...
  try {
    ...
    if (vendor && vendor.email) {
      await sendNotificationEmail(vendor.email, `Your service "${service.name}" has been approved and is now published.`);
    }
...
export const approveCard = async (req, res) => {
  ...
  try {
    ...
    if (vendor && vendor.email) {
      await sendNotificationEmail(vendor.email, `Your wedding card template has been approved and is now published.`);
    }
...
export const cancelBooking = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (user && user.email) await sendNotificationEmail(user.email, `Your booking has been canceled by the admin.`);
      if (vendor && vendor.email) await sendNotificationEmail(vendor.email, `A booking has been canceled by the admin.`);
...
export const cancelUserBooking = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const vendor = await User.findById(booking.vendor_id);
      if (vendor && vendor.email) {
        await sendNotificationEmail(vendor.email, `A booking has been canceled by the user.`);
      }
...
export const revertServiceToPending = async (req, res) => {
  ...
  try {
    ...
    const vendor = await User.findById(service.vendor_id);
    if (vendor && vendor.email) {
      await sendNotificationEmail(vendor.email, `Your service "${service.name}" has been reverted to pending status for review.`);
    }
...
export const revertCardToPending = async (req, res) => {
  ...
  try {
    ...
    const vendor = await User.findById(card.vendor_id);
    if (vendor && vendor.email) {
      await sendNotificationEmail(vendor.email, `Your card template has been reverted to pending status for review.`);
    }
...
export const createBooking = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const user = await User.findById(userId);
      if (vendor && vendor.email) {
        await sendNotificationEmail(vendor.email, `A new booking has been requested by ${user.full_name}.`);
      }
  ...
  try {
    ...
    try {
      ...
      for (const vendorId of vendorIds) {
        const vendor = await User.findById(vendorId);
        if (vendor && vendor.email) {
          await sendNotificationEmail(vendor.email, `New bookings have been requested from an estimation.`);
        }
      }
