...
export const normalizeVendorRequest = (req, res, next) => {
  try {
    ...
    if (req.headers["content-type"]?.includes("multipart/form-data")) {
      ...
      // Normalize Indian phone numbers if present
      if (req.normalizedBody.phone) {
        const norm = normalizeIndianPhone(req.normalizedBody.phone)
        if (norm) req.normalizedBody.phone = norm
      }
      if (vendorRequest.whatsapp_number) {
        const norm = normalizeIndianPhone(vendorRequest.whatsapp_number)
        if (norm) vendorRequest.whatsapp_number = norm
      }
      if (vendorRequest.phone_whatsapp) {
        const norm = normalizeIndianPhone(vendorRequest.phone_whatsapp)
        if (norm) vendorRequest.phone_whatsapp = norm
      }
