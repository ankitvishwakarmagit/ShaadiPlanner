...
const Listings = () => {
  ...
  useEffect(() => {
    document.title = searchQuery
      ? `Search Results for "${searchQuery}" | ShaadiPlanner`
      : categoryParam
      ? `${categoryParam} | ShaadiPlanner`
      : "Service Listings | ShaadiPlanner";

    const loadFilters = async () => {
      try {
        const filterData = await fetchServiceFilters();
        const cities = filterData.cities || [];
        const requiredCities = ["Mumbai", "Delhi", "Bengaluru", "Chennai"];
