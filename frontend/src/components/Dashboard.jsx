            ...
            <div className="col-md-4 mb-4">
              <div className="stats-card">
                ...
                <div className="stats-footer">
                  <i className="fas fa-money-bill-wave me-1 text-success"></i> INR {stats.totalEstimationCost?.toLocaleString() || 0}
                </div>
              </div>
            </div>
            <div className="col-md-4 mb-4">
              ...
              dashboardData.estimations.map((estimation) => (
                <div key={estimation.estimation_id} className="mb-4">
                      ...
                      <tbody>
                        {estimation.services.map((service) => (
                          <tr key={service.service_id}>
                            ...
                            <td>INR {service.package_price.toLocaleString()}</td>
                            <td>{service.quantity}</td>
                            <td>INR {(service.package_price * service.quantity).toLocaleString()}</td>
                            <td>
                        ...
                        {estimation.cards.map((card) => (
                          <tr key={card.card_id}>
                            <td>{card.name}</td>
                            <td>N/A</td>
                            <td>INR {card.price_per_card.toLocaleString()}</td>
                            <td>{card.quantity}</td>
                            <td>INR {(card.price_per_card * card.quantity).toLocaleString()}</td>
                            <td>
                              <div className="action-icons">
                                <Button
                                  variant="link"
                  ...
                  <div className="d-flex justify-content-between mt-3">
                    <h5>Total: INR {estimation.total_cost.toLocaleString()}</h5>
                  </div>
                </div>
              ))
            ) : (
                  ...
                  {item.review ? (
                    <div className="mt-3 p-3 bg-light rounded">
                      <div className="d-flex align-items-center mb-2">
                        ...
                        <span className="text-warning">{Array(item.review.rating).fill("â˜…").join("")}</span>
